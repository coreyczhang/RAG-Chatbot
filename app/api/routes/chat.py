...
def chat(req: ChatRequest):
    try:
        ...
        retrieved_docs = retrieval.similarity_search(query=req.query, k=min(req.k, settings.max_k), filter=req.filter, match_threshold=req.match_threshold)
        ...
        answer = chat_service.answer(query=req.query, retrieved_docs=retrieved_docs, model=model, max_output_tokens=max_output_tokens, temperature=temperature)
        
        sources = [SourceChunk(content_preview=(d.page_content or "")[:500], metadata=d.metadata or {}) for d in retrieved_docs]
